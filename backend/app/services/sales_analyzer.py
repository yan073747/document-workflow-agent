...
def analyze_sales_file(file_path: str | Path) -> SalesAnalysis:
    ...
    top_regions = _top_items(revenue_by_region, quantity_by_region)
    top_categories = _top_items(revenue_by_category, quantity_by_category)
    customer_mix = _customer_mix(revenue_by_customer, total_revenue)
    insights = _build_insights(total_revenue, order_count, top_regions, top_categories, customer_mix)
