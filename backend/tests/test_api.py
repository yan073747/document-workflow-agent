...
def auth_headers(client: TestClient, email: str = "demo@example.com", password: str = "demo123456") -> dict[str, str]:
...
def test_create_confirm_and_read_report() -> None:
    ...
    confirm_response = client.post(f"/api/tasks/{task_id}/confirm", headers=headers)
    assert confirm_response.status_code == 200
    assert confirm_response.json()["task"]["status"] == "completed"

    report_response = client.get(f"/api/tasks/{task_id}/report.md", headers=headers)
    ...
    pdf_response = client.get(f"/api/tasks/{task_id}/report.pdf", headers=headers)
...
def test_failed_task_can_retry_with_replacement_file() -> None:
    ...
    failed_response = client.post(f"/api/tasks/{task_id}/confirm", headers=headers)
    ...
    completed_response = client.post(f"/api/tasks/{task_id}/confirm", headers=headers)
