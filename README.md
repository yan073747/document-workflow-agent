...
面向企业办公场景的 AI Agent 作品集项目。用户上传销售 Excel 并输入任务目标，例如“分析本月销售数据并生成经营报告”，系统会创建任务，由 Planner、Data Analyst、Writer、Reviewer 等角色协作完成分析、报告生成、质量审核和结果归档。
...
- 无 Key 稳定演示：第一版使用本地规则跑通完整流程，后续接入 LLM。
...
Planner Agent 和 Writer Agent 会优先调用远程 LLM。如果没有 Key、网络失败、响应格式错误，系统会自动回退到本地规则，保证面试演示不中断。
...
- 设计失败重试、人工确认和无 Key 本地规则回退机制，保证项目在面试演示中稳定运行。
